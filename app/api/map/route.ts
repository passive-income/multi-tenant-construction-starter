...
export async function GET(req: Request) {
  try {
    ...
    let loc: any = null
    try {
      const raw = await fs.readFile(path.join(process.cwd(), 'data', 'static-mueller.json'), 'utf8')
    ...
    if (!resolvedQuery && !presetCoords) {
      return NextResponse.json({ error: 'No address provided and no default location found' }, { status: 400 })
    }

    const companyName = loc?.companyName || loc?.title || loc?.name || 'location'
    ...
    try {
      ...
      if (stat && stat.size > 0) {
        const meta = await fs.readFile(metaPath, 'utf8').then((m) => JSON.parse(m)).catch(() => undefined)
        return NextResponse.json({ url: `/generated-maps/${fileName}`, meta, cache: true })
      }
    } catch (_) {
      // miss; proceed
    }
    ...
    if (typeof lat !== 'number' || typeof lon !== 'number') {
      ...
      if (resolvedQuery && !/germany/i.test(resolvedQuery)) queries.push(resolvedQuery + ', Germany')
    ...
    try {
      ...
      return NextResponse.json({ error: 'Failed to fetch static map', detail: String(e), mapUrl: staticMapUrl }, { status: 500 })
    }
  } catch (err: any) {
    return NextResponse.json({ error: 'Unhandled server error', detail: String(err), stack: err?.stack }, { status: 500 })
  }
}
