...
export function MainSection({ data }: MainSectionProps) {
  ...
  const beforeAfterSources = sliderSlides.map((s: any) => s.image).filter(Boolean)
  ...
  if (beforeAfterArray && beforeAfterArray.length > 0) {
    ...
    const makeItemsFromSlides = (start: number, count: number) => {
      const items: any[] = []
      for (let i = start; i < Math.min(sliderSlides.length - 1, start + count); i++) {
        ...
        <AnimatedSection className="py-12 bg-muted/10">
          <div className="container">
            <div className="text-center mb-8">
              ...
              <p className="text-muted-foreground mt-2">Drei Paare in einer Zeile (responsive).</p>
            </div>
