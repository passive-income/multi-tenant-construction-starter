...
export async function TeamSection({ clientId, dataset = "production" }: TeamSectionProps) {
  ...
  if ((!teamMembers || teamMembers.length === 0) && !intendedStatic) {
    try {
      ...
      teamMembers = (json as any)?.teamMembers || [];
      if (teamMembers && teamMembers.length > 0) sourceNote = "Using static JSON fallback (Sanity missing)";
    } catch (e) {
      teamMembers = [];
    }
  }
  ...
  return (
    <section className="py-16">
      <div className="container mx-auto px-4 sm:px-6 lg:px-8">
        {sourceNote && <p className="text-sm text-muted-foreground text-center mb-4">{sourceNote}</p>}
