...
export default function ClsLogger() {
  useEffect(() => {
    ...
    const shouldLog = process.env.NEXT_PUBLIC_LOG_CLS === "1" || process.env.NODE_ENV !== "production";
    if (!shouldLog) return;
    ...
    const observer = new PerformanceObserver((list) => {
      for (const entry of list.getEntries() as PerformanceEntry[]) {
        // @ts-ignore layout-shift specific fields
        ...
        const selectors = sources
          .map((s: any) => getElementSelector(s?.node))
          .filter(Boolean);
