...
const GEOAPIFY_KEY = process.env.GEOAPIFY_API_KEY || process.env.NEXT_PUBLIC_GEOAPIFY_API_KEY
...
async function geocode(query: string) {
  ...
  if (!r) return null
  return { lat: r.lat, lon: r.lon, confidence: r.rank?.confidence ?? 0, result: r }
}
...
function pickLatLonFromLocation(loc: any) {
  ...
  if (!loc) return null
  if (typeof loc.lat === 'number' && typeof loc.lon === 'number') return { lat: loc.lat, lon: loc.lon }
  if (loc.coordinates && Array.isArray(loc.coordinates) && loc.coordinates.length >= 2) return { lon: loc.coordinates[0], lat: loc.coordinates[1] }
  if (loc.address && typeof loc.address.lat === 'number' && typeof loc.address.lon === 'number') return { lat: loc.address.lat, lon: loc.address.lon }
  return null
}
...
async function processLocation(loc: any, index: number) {
  ...
  if (!coords) {
    const q = buildQueryFromLocation(loc) || (loc.address && loc.address.formatted) || ''
  ...
  const key = hashKey(`${coords.lat},${coords.lon}|${mapSpec.style}|${mapSpec.width}x${mapSpec.height}`)
  ...
  await fs.writeFile(path.join(OUT_DIR, `${fileName}.json`), JSON.stringify(meta, null, 2), 'utf8')
...
async function main() {
  if (!GEOAPIFY_KEY) {
    console.error('GEOAPIFY_API_KEY or NEXT_PUBLIC_GEOAPIFY_API_KEY is required in env')
    process.exit(2)
  }
  ...
  const candidates = data?.footer?.locations || data?.locations || data?.companies || data?.sites || []
  if (!Array.isArray(candidates) || candidates.length === 0) {
    throw new Error('No locations array found in data/static-mueller.json (expected footer.locations or locations)')
  }
  ...
  for (let i = 0; i < candidates.length; i++) {
    try {
      ...
      const hasAddress = loc.address || loc.street || loc.city || loc.lat || loc.lon
      if (!hasAddress) continue
      // generate one map per location
      const meta = await processLocation(loc, i)
      results.push(meta)
    } catch (err) {
      console.error('Skipped location', i, err instanceof Error ? err.message : err)
    }
  }

  console.log('Done. Generated maps:', results.map((r: any) => r.file))
}
